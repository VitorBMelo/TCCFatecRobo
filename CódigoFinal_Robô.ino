...
void radar_APO1(){ // Radar de ativação por objeto, dianteiro
  if(monitora_Dianteira == true && D7 == false){
    ...
    if((dist_APO1 < dist_Ref0) && (dist_APO1 != 0)){ // Verificação se objeto há algum objeto no caminho, no raio de alcance dref0
...
void reconhecimento_Zona(){
  if(ativa_APO == true && curva_Liberada == false && manobra == false && memoria_EQ == false && memoria_DT == false && D7 == false){
    ...
    Serial.println("reconhecimento de Zona ON");  //Segue a obtenção das diagonais e retas essenciais para movimentação
    S1.write(ang_1);
    delay(delay0);
    digitalWrite(trigger1,1);
